...
import LeftSidebar from "../components/left_SideBar"; // Adjust the path if needed

// Initial chart data for different timeframes
const initialChartData = {
  "1H": Array.from({ length: 15 }, (_, i) => ({
    time: new Date(Date.now() - (14 - i) * 2 * 60 * 1000).toLocaleTimeString(
      "en-US",
  ...
  "4H": Array.from({ length: 8 }, (_, i) => ({
    time: new Date(Date.now() - (7 - i) * 30 * 60 * 1000).toLocaleTimeString(
      "en-US",
      { hour: "2-digit", minute: "2-digit", hour12: false }
    ),
    price: 50000 + (Math.random() - 0.5) * 1000,
...
const getNextTime = (lastTime, timeframe) => {
  ...
  if (timeframe === "1W") {
    ...
    return lastDate.toLocaleTimeString("en-US", {
      hour: "2-digit",
      minute: "2-digit",
      hour12: false,
    });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setChartData((prevData) => {
        ...
        currentData.push({
          time: newTime,
          price: Math.max(40000, Math.min(60000, newPrice)),
        });
              ...
              <div>
                ...
                <p className="eth-usd-price">
                  ...
                  {chartData[selectedTimeframe][
                    chartData[selectedTimeframe].length - 1
                  ].price.toFixed(2)}{" "}
              ...
              <ResponsiveContainer width="100%" height={400}>
                ...
                >
                  ...
                  <XAxis
                    ...
                    interval={Math.floor(
                      chartData[selectedTimeframe].length / 5
                    )} // Adjust tick density
                  />
                  <YAxis
                    stroke="#9CA3AF"
